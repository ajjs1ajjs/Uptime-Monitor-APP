...
```bash
# Оновити з Git репозиторію
cd /opt/uptime-monitor && sudo git pull && sudo systemctl restart uptime-monitor

# Перевірити статус після оновлення
sudo systemctl status uptime-monitor

# Якщо виникла помилка "dubious ownership"
sudo git config --global --add safe.directory /opt/uptime-monitor
```

### Повне перевстановлення (якщо щось зламалося)
```bash
# Бекап бази даних перед перевстановленням
sudo cp /opt/uptime-monitor/Uptime_Robot/sites.db /tmp/sites.db

# Перевстановлення
curl -fsSL https://raw.githubusercontent.com/ajjs1ajjs/Uptime-Monitor-APP/main/install.sh | sudo bash

# Відновити базу даних
sudo cp /tmp/sites.db /opt/uptime-monitor/Uptime_Robot/sites.db
sudo systemctl restart uptime-monitor
```
